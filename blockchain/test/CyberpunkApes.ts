...
contract('CyberpunkApeExecutives', (accounts) => {
    beforeEach(async () => {
        CyberpunkApeExecutives.link(
            'VerifySignature',
            (await Signer.new()).address
        );
    });
    ...
    it('sets presale mint', async () => {
        ...
        const cyberpunkApeExecutivesInstance = await CyberpunkApeExecutives.new(
            5500,
            1000,
            10,
            3,
        ...
        const { 0: startDate, 1: endDate } =
            await cyberpunkApeExecutivesInstance.presaleMint();
    ...
    it('sets public mint', async () => {
        ...
        const { 0: startDate, 1: maxPerTransaction } =
            await cyberpunkApeExecutivesInstance.publicMint();
    ...
    it('sets baseURI', async () => {
        ...
        await truffleAssert.fails(cyberpunkApeExecutivesInstance.tokenURI(2));
    });
    ...
    it('updates presale mint', async () => {
        ...
        let { 0: startDate, 1: endDate } =
            await cyberpunkApeExecutivesInstance.presaleMint();

        assert.equal(startDate.toNumber(), later, 'Invalid startDate');
        assert.equal(endDate.toNumber(), later, 'Invalid endDate');

        await cyberpunkApeExecutivesInstance.updatePresaleMint(
            now,
            later + 2,
            1000
        );
    ...
    it('updates public mint', async () => {
        ...
        let { 0: startDate, 1: maxPerTransaction } =
            await cyberpunkApeExecutivesInstance.publicMint();

        assert.equal(startDate.toNumber(), later, 'Invalid startDate');
        assert.equal(
            maxPerTransaction.toNumber(),
            10,
            'Invalid public sale max'
        );

        await cyberpunkApeExecutivesInstance.updatePublicMint(2, now);
        ({ 0: startDate, 1: maxPerTransaction } =
            await cyberpunkApeExecutivesInstance.publicMint());
    ...
    it('sets the owner', async () => {
        ...
        assert.equal(
            await cyberpunkApeExecutivesInstance.owner(),
            accounts[0],
    ...
    it('mints publically', async () => {
        ...
        await cyberpunkApeExecutivesInstance.mint(3, {
            value: '9',
            from: accounts[1],
        });

        const newBalance = await (
            await cyberpunkApeExecutivesInstance.balanceOf(accounts[1])
        ).toNumber();
    ...
    it('cannot mint more than transaction maximum', async () => {
        ...
        await truffleAssert.fails(
            cyberpunkApeExecutivesInstance.mint(6, {
                value: '18',
                from: accounts[1],
            })
        );
    ...
    it('mints presale', async () => {
        ...
        const hash = await cyberpunkApeExecutivesInstance.getPremintHash(
            trans.minter,
            trans.quantity,
            trans.nonce
        );
        ...
        await cyberpunkApeExecutivesInstance.premint(
            trans.quantity,
            trans.nonce,
            signature,
            { from: accounts[1], value: '9' }
        );
        assert.equal(
            (
                await cyberpunkApeExecutivesInstance.balanceOf(accounts[1])
        ...
        assert.equal(
            (
                await cyberpunkApeExecutivesInstance.getPresaleMints(
                    accounts[1]
                )
            ).toNumber(),
            3
    ...
    it('mints presale with new signer', async () => {
        ...
        await truffleAssert.fails(
            cyberpunkApeExecutivesInstance.setSigner(signerAccount2.address, {
                from: accounts[1],
            }) // wrong user
        );
    ...
    it('cannot mint more presale than limit', async () => {
        ...
        await truffleAssert.fails(
            cyberpunkApeExecutivesInstance.premint(
                trans.quantity,
                trans.nonce,
                signature,
                { from: accounts[1] }
    ...
    it('cannot mint when no mints active', async () => {
        ...
        await truffleAssert.fails(
            cyberpunkApeExecutivesInstance.mint(3, {
                value: '9',
                from: accounts[1],
            }) // PUBLIC MINT
        );
    ...
    it('can retrieve the latest nonce for a user', async () => {
        ...
        const latestNonce = await cyberpunkApeExecutivesInstance.lastMintNonce(
            accounts[1]
        );
    ...
    it('can retrieve the number minted by a user', async () => {
        ...
        const transferEvents =
            await cyberpunkApeExecutivesInstance.getPastEvents('Transfer');
        const mintEvents = transferEvents.filter(
            (e) =>
                e.returnValues.from ===
                    '0x0000000000000000000000000000000000000000' &&
                e.returnValues.to === accounts[1]
        );
    ...
    it('withdraws', async () => {
        ...
        await cyberpunkApeExecutivesInstance.mint(5, {
            value: '15',
            from: accounts[1],
        });
        const balance = await web3.eth.getBalance(
            cyberpunkApeExecutivesInstance.address
        );
        ...
        await cyberpunkApeExecutivesInstance.withdraw(15, {
            from: accounts[0],
        });
